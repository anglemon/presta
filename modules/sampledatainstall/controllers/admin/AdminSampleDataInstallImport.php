...
class AdminSampleDataInstallImportController extends ModuleAdminController {
	...
	public function postProcess()
	{
		...
		{
			$this->clearTemporaryConfigEntries();
            ...
            $this->clearTemporaryFolder();
			$this->clearTemporaryConfigEntries();
			die(Tools::jsonEncode(array('status' => 'import_end')));
		}
	...
	**/
	...
	public function filterFields($class_name, $fields, $line)
	{
		if (count($fields) != count($line)) {
            $this->clearTemporaryFolder();
            die(Tools::jsonEncode(array('status' => 'error', 'responseText' => sprintf($this->l('Look like your %s file structure is incorect'), $class_name))));
        }
	...
	protected function clearTemporaryConfigEntries()
	{
	...
	public function getCurrentStatusAjax()
	{
		...
		die(Tools::JsonEncode(array('responseText' => $message)));
	}

    protected function clearTemporaryFolder()
    {
        $path = new Sampledatainstall();
